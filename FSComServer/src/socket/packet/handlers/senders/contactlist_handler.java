...
public class contactlist_handler extends send_handler {
	...
	private String getContactList() 
	{
		...
		for(int i=0;i<v_cont.size();i++)
		{
			...
			pck += v_cont.get(i);
			pck += "//.//";
			pck += DatabaseTransactions.IntegerQuery("acc_blocked", "blocked", acc_contact_get);
			pck += "//.//";
			...
			pck += "//.//";
			pck += DatabaseTransactions.StringQuery("account", "phr_perso", "uid = '" + v_cont.get(i) + "'");
			pck += "//.//";
			if(SessionHandler.isConnected(v_cont.get(i)))
				pck += SessionHandler.getContactByUID(v_cont.get(i)).getStatus();
			else
				pck += "0";
			pck += "///.///";
		}		
